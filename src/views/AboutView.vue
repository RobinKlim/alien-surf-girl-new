<template>
  <div class="about">
    <CDCaseMobile
      v-if="mobileOrTablet && !hasRenderedCDCaseMobile"
      @cd-clicked="
        () => {
    ...
    <div v-if="mobileOrTablet" class="mobileContainer">
      ...
      <div class="card-deck">
        <img
          class="single-card"
          id="cardJonas"
          src="@/assets/PICS/Einzelfotos-Sammelkarten-Kartenformat/Spielkarten_Jonas.png"
          @touchstart="saveClickPosition"
          @touchmove="moveImage"
          @click="test"
        />
        <img
          class="single-card"
          id="cardCyrus"
          src="@/assets/PICS/Einzelfotos-Sammelkarten-Kartenformat/Spielkarten_Cyrus.png"
          @touchstart="saveClickPosition"
          @touchmove="moveImage"
          @click="test"
        />
        <img
          class="single-card"
          id="cardPhilipp"
          src="@/assets/PICS/Einzelfotos-Sammelkarten-Kartenformat/Spielkarten_Philipp.png"
          @touchstart="saveClickPosition"
          @touchmove="moveImage"
          @click="test"
        />
        <img
          class="single-card"
          id="cardMorten"
          src="@/assets/PICS/Einzelfotos-Sammelkarten-Kartenformat/Spielkarten_Morten.png"
          @touchstart="saveClickPosition"
          @touchmove="moveImage"
          @click="test"
        />
        <img
          class="single-card"
          id="cardRobin"
          src="@/assets/PICS/Einzelfotos-Sammelkarten-Kartenformat/Spielkarten_Robin.png"
          @touchstart="saveClickPosition"
          @touchmove="moveImage"
          @click="test"
        />
    ...
    <div v-else>
      <div class="bandshotContainer">
        <img
          class="bandShot bandShotClean"
          src="@/assets/PICS/Bandshot-black-Outfit-freigestellt-clean.png"
          alt="Bandfoto mit in schwarzen Outfits freigestellt"
        />
        <img
          v-if="!hoveredSingle"
          class="bandShot bandShotGlitch"
          src="@/assets/PICS/Bandshot-black-Outfit-freigestellt-Glitch.png"
          alt="Bandfoto mit in schwarzen Outfits freigestellt mit rot-blauen Glitch Effekt"
        />
        <p>{{ informationText }}</p>
      </div>
      <div class="card-desktop">
        <img
          class="single-card-desktop"
          src="@/assets/PICS/Einzelfotos-Sammelkarten-Kartenformat/Spielkarten_Jonas.png"
          data-card="Jonas"
          @mouseenter="togglePicture"
          @mouseleave="leavePicture"
        />
        <img
          class="single-card-desktop"
          src="@/assets/PICS/Einzelfotos-Sammelkarten-Kartenformat/Spielkarten_Cyrus.png"
...
export default {
  ...
  data() {
    return {
      ...
      intervalId: null,
      informationText:
        "Alien Surf Girl bringt verspielt Nu-Disco Pop in das Gewand einer 2000er Band. Weiß-lackierte Fingernägel, die Eleganz von Feinripp und Silber-Schmuck, kombiniert mit einer weichen und zärtlichen Art.",
    };
  },
  mounted() {
  ...
  methods: {
    reorderImage(element) {
      ...
      for (let i = 0; i < allSingleCardsArr.length; i++) {
        const zIndex = getComputedStyle(allSingleCardsArr[i]).getPropertyValue(
          "z-index"
        );
        if (zIndex == 1) {
    ...
    test(e) {
      ...
      if (this.zIndexTop == 5 && !this.cardIsMoving) {
        this.cardIsMoving = true;
        this.clickedImage = e.target;
        this.zIndexTop = getComputedStyle(this.clickedImage).getPropertyValue(
          "z-index"
        );
    ...
    leavePicture() {
      ...
      document.querySelector(".bandShotClean").src =
        this.preloadedImages.bandShot;
    },
